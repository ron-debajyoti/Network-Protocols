...
{
    bzero(d->buf, BUFSIZE);
    int n = recvfrom(sockfd, d, sizeof(datagram), 0,(struct sockaddr *) &serveraddr, (socklen_t*)&serverlen);
    return n;
}
...
{
    int n = sendto(sockfd, d, sizeof(datagram), 0,(struct sockaddr *) &serveraddr, serverlen);
...
void client_details(int* sockfd1,char* filename,int* nochunks1,int* size11,char** argv)
{
   ...
   bzero(buf, BUFSIZE);
   int nochunks = (int)(size/1024)+( (size%1024==0)? 0 : 1) ;
...
// updating window based on 4 states
void update_window(window* z,int nochunks)
{
        ...
        else if(z->state==3)
        {
            ...
            if(z->windsize<=z->ssthresh) z->cwnd=z->windsize+(z->ack-z->curr);
            else    z->cwnd=z->windsize+1;
        }
        z->state=-1;
...
void client_file_check(char* filename,int sockfd,struct sockaddr * &serveraddr,int serverlen)
{
    ...
    MD5_Init (&mdContext);
    while ((bytes = fread (data, 1, BUFSIZE, inFile)) != 0) MD5_Update (&mdContext, data, bytes);
    ...
    for(i = 0; i < MD5_DIGEST_LENGTH; i++) printf("%02x", crev[i]);
    printf (" %s\n", filename);
    if(strstr((const char*)c,(const char*)crev)==NULL) printf("MD5  not Matched\n");
